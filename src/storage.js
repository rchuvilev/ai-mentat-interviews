...
// Config, credentials and preferences live at
// <filesystem root>/.hexstack-app/ai-mentat-interviews/data for every build
// type. resolveDataDir falls back to ~/.hexstack-app/<app>/data when the root
// is not user-writable (see sdk/utils/data-dir.js).
const { resolveDataDir } = require('../sdk/utils/data-dir');

function getConfigDir() {
    return resolveDataDir('ai-mentat-interviews');
}
...
// Single source of truth for a day's limit entry.
//
// 🔴 This exists because getTodayLimits() and incrementLimitCount() each built
// their own version of this object, and they diverged: incrementLimitCount
// created an entry with only { date, flash, flashLite } and NO groq/gemini
// fields. Any caller reading the returned entry — e.g. entry.groq['qwen3-32b']
// — threw "Cannot read properties of undefined". It only appeared to work
// because incrementCharUsage happens to call getTodayLimits() first, which
// backfills the missing fields as a side effect.
//
// Two constructors for one shape will always drift. One factory cannot.
function makeTodayEntry(date) {
    return {
        date,
        flash: { count: 0 },
        flashLite: { count: 0 },
...
// Backfill fields added after an entry was first written, so an entry saved by
// an older build does not crash a newer reader. Returns true if anything
// changed, so the caller only persists when there is something to persist.
function ensureEntryShape(entry) {
    const ref = makeTodayEntry(entry.date);
    let changed = false;
    for (const key of ['flash', 'flashLite', 'groq', 'gemini']) {
        if (!entry[key]) { entry[key] = ref[key]; changed = true; }
    }
    for (const provider of ['groq', 'gemini']) {
        for (const model of Object.keys(ref[provider])) {
            if (!entry[provider][model]) { entry[provider][model] = ref[provider][model]; changed = true; }
        }
    }
    return changed;
}
...
function getTodayLimits() {
    ...
    if (todayEntry) {
        // Only write when a field was actually missing — an unconditional
        // setLimits() here meant every read caused a disk write.
        if (ensureEntryShape(todayEntry)) setLimits(limits);
        return todayEntry;
    }

    // No entry for today - drop stale days and create one
    limits.data = limits.data.filter(entry => entry.date === today);
    const newEntry = makeTodayEntry(today);
...
function incrementLimitCount(model) {
    ...
    if (!todayEntry) {
        // Drop stale days and create a FULL entry via the shared factory.
        // Building a partial { date, flash, flashLite } here was the bug:
        // callers reading entry.groq[...] on the returned object threw.
        limits.data = [];
        todayEntry = makeTodayEntry(today);
        limits.data.push(todayEntry);
    } else {
        limits.data = limits.data.filter(entry => entry.date === today);
        // An entry written by an older build may predate groq/gemini.
        ensureEntryShape(todayEntry);
    }
