...
export class MainView extends LitElement {
    ...
    static properties = {
        ...
        // Claude Code state
        _claudeModel: { state: true },
        _claudeTranscription: { state: true },
    };
    ...
    constructor() {
        ...
        this._claudeTranscription = 'whisper';
    ...
    async _loadFromStorage() {
        try {
            ...
            this._claudeModel = prefs.claudeModel || 'sonnet';
            this._claudeTranscription = prefs.claudeTranscription || 'whisper';
    ...
    async _saveClaudeModel(val) {
        this._claudeModel = val;
        await cheatingDaddy.storage.updatePreference('claudeModel', val);
        this.requestUpdate();
    }

    async _saveClaudeTranscription(val) {
        this._claudeTranscription = val;
        await cheatingDaddy.storage.updatePreference('claudeTranscription', val);
        this.requestUpdate();
    }
    ...
    _handleStart() {
        ...
        } else if (this._mode === 'claudecode') {
            // Whisper transcription is fully local; Gemini transcription needs the key.
            if (this._claudeTranscription === 'gemini' && !this._geminiKey.trim()) {
                this._keyError = true;
                this.requestUpdate();
                return;
            }
    ...
    triggerApiKeyError() {
        this._keyError = this._mode === 'byok' || (this._mode === 'claudecode' && this._claudeTranscription === 'gemini');
        this.requestUpdate();
        setTimeout(() => {
            this._tokenError = false;
            this._keyError = false;
    ...
    _renderByokMode() {
        return html`
            ...
            <div class="mode-links">
                ...
                <button class="mode-link" @click=${() => this._saveMode('claudecode')}>Use Claude Code</button>
            </div>
        `;
    }
    ...
    _renderClaudeCodeMode() {
        const codeStyle = 'font-family: var(--font-mono); font-size: 11px; background: var(--bg-elevated); padding: 1px 4px; border-radius: 3px;';
        return html`
            <div class="form-group">
                <label class="form-label">Claude Model</label>
                <select .value=${this._claudeModel} @change=${e => this._saveClaudeModel(e.target.value)}>
                    <option value="sonnet" ?selected=${this._claudeModel === 'sonnet'}>Sonnet (recommended — fast)</option>
                    <option value="opus" ?selected=${this._claudeModel === 'opus'}>Opus (most capable)</option>
                    <option value="haiku" ?selected=${this._claudeModel === 'haiku'}>Haiku (fastest)</option>
                </select>
                <div class="form-hint">Uses your local <code style=${codeStyle}>claude</code> CLI login — no API key, no per-token cost</div>
            </div>

            <div class="form-group">
                <label class="form-label">Transcription</label>
                <select .value=${this._claudeTranscription} @change=${e => this._saveClaudeTranscription(e.target.value)}>
                    <option value="whisper" ?selected=${this._claudeTranscription === 'whisper'}>Local Whisper (offline)</option>
                    <option value="gemini" ?selected=${this._claudeTranscription === 'gemini'}>Gemini (needs Gemini API key)</option>
                </select>
                <div class="form-hint">How speech becomes text before it's sent to Claude</div>
            </div>

            ${
                this._claudeTranscription === 'gemini'
                    ? html`
                          <div class="form-group">
                              <label class="form-label">Gemini API Key</label>
                              <input
                                  type="password"
                                  placeholder="Used for transcription only"
                                  class=${this._keyError ? 'input-error' : ''}
                                  .value=${this._geminiKey}
                                  @input=${e => this._saveGeminiKey(e.target.value)}
                              />
                              <div class="form-hint">
                    ...
                    : html`
                          <div class="form-group">
                              ...
                              <select .value=${this._whisperModel} @change=${e => this._saveWhisperModel(e.target.value)}>
                                  ...
                                  <option value="Xenova/whisper-small" ?selected=${this._whisperModel === 'Xenova/whisper-small'}>
                                      Small (recommended)
                                  </option>
                    ...
                    this._mode === 'local'
                        ...
                        : html`
                              <div class="page-title">
                                  Cheating Daddy <span class="mode-suffix">${this._mode === 'claudecode' ? 'Claude Code' : 'BYOK'}</span>
                              </div>
                          `
                }
                <div class="page-subtitle">
                    ${
                        this._mode === 'byok'
                            ...
                            : this._mode === 'claudecode'
                              ? 'Answers from your local Claude Code'
                              : 'Run models locally on your machine'
                    }
                </div>

                <!-- Cloud mode render branch intentionally disabled. -->
                ${this._mode === 'byok' ? this._renderByokMode() : ''} ${this._mode === 'claudecode' ? this._renderClaudeCodeMode() : ''}
