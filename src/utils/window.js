...
function createWindow(sendToRenderer, geminiSessionRef) {
    ...
    session.defaultSession.setDisplayMediaRequestHandler(
        (request, callback) => {
            // Denying a video request (empty streams) throws synchronously in this
            // Electron version; swallow it so it never becomes an unhandled rejection.
            // getDisplayMedia() then rejects on the renderer, where startCapture()
            // handles it (screenshots disabled; audio/mic capture is unaffected).
            const deny = () => {
                try {
                    callback({});
                } catch (e) {
                    /* request denied — nothing more to do */
                }
            };
            desktopCapturer
                ...
                .then(sources => {
                    if (sources && sources.length > 0) {
                        ...
                        // No screen source (e.g. Screen Recording permission not granted).
                        deny();
                    }
                })
                .catch(err => {
                    console.warn('Failed to get screen sources for display media:', err && err.message ? err.message : err);
                    deny();
                });
        },
