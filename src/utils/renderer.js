...
async function initializeClaudeCode(profile = 'interview', language = 'en-US') {
    const prefs = await storage.getPreferences();
    const transcription = prefs.claudeTranscription || 'whisper';
    // Gemini transcription needs the Gemini API key; Whisper is fully local.
    const apiKey = transcription === 'gemini' ? await storage.getApiKey().catch(() => '') : '';

    const success = await ipcRenderer.invoke('initialize-claude-code', {
        model: prefs.claudeModel || 'sonnet',
        transcription,
    ...
    if (success) {
        cheatingDaddy.setStatus('Claude Code Live');
        return true;
    } else {
        cheatingDaddy.setStatus('error');
        return false;
...
async function startCapture(screenshotIntervalSeconds = 5, imageQuality = 'medium') {
    ...
    try {
        if (isMacOS) {
            console.log('Starting macOS capture, audio mode:', audioMode);

            // 1) Microphone FIRST and independently. Screen Recording permission is
            //    what SystemAudioDump and getDisplayMedia need; the mic only needs
            //    Microphone permission. Capturing it first means a denied/failed
            //    screen capture can no longer prevent the mic from being heard.
            ...
            // 2) System (speaker) audio via SystemAudioDump — only when actually needed.
            if (audioMode === 'speaker_only' || audioMode === 'both') {
                const audioResult = await ipcRenderer.invoke('start-macos-audio');
                if (!audioResult.success) {
                    console.warn('Failed to start macOS system audio capture:', audioResult.error);
                }
            }

            // 3) Screen capture for on-demand screenshots — non-fatal. A failure here
            //    (e.g. missing Screen Recording permission) must not stop audio capture.
            try {
                ...
                console.warn('macOS screen capture unavailable (screenshots disabled):', screenError);
            }
        } else if (isLinux) {
