...
// Where finished transcripts are routed. Defaults to Ollama, but another
// backend (e.g. Claude Code) can take over the transcription pipeline by
// installing its own handler via setResponseHandler().
...
async function loadWhisperPipeline(modelName) {
    ...
    try {
        ...
        whisperPipeline = await pipeline('automatic-speech-recognition', modelName, {
            dtype: 'q8',
            device: 'auto',
            // Disable onnxruntime's CPU memory arena. Its power-of-two arena
            // extension requests a ~2GB aligned block, which Electron's
            // PartitionAlloc rejects with SIGTRAP, crashing the whole process
            // during inference. Without the arena, ORT allocates tensors at
            // their real (KB–MB) sizes, which PartitionAlloc serves fine.
...
async function handleSpeechEnd(audioData) {
    try {
        ...
        console.log('[LocalAI] handleSpeechEnd: transcribing', audioData.length, 'bytes');
        const transcription = await transcribeAudio(audioData);
        console.log('[LocalAI] handleSpeechEnd: transcription =', JSON.stringify(transcription));
        ...
        console.log('[LocalAI] handleSpeechEnd: routing transcript to response handler');
        await (responseHandler || sendToOllama)(transcription);
        console.log('[LocalAI] handleSpeechEnd: response handler returned');
    } catch (error) {
        // Fire-and-forget caller means an unhandled throw here would surface as an
        // unhandled rejection; log it instead so it cannot silently take down state.
        console.error('[LocalAI] handleSpeechEnd error:', error && error.stack ? error.stack : error);
        sendToRenderer('update-status', 'Transcription error: ' + (error && error.message ? error.message : error));
    }
}
...
// Bring up ONLY the Whisper + VAD transcription pipeline (no Ollama). Used by
// other response backends (e.g. Claude Code) that want local transcription but
// generate answers elsewhere. Route transcripts with setResponseHandler().
...
// Install a custom transcript sink. Pass null / call resetResponseHandler to
// restore the default Ollama handler.
function setResponseHandler(fn) {
    responseHandler = typeof fn === 'function' ? fn : null;
}

function resetResponseHandler() {
...
async function initializeLocalSession(ollamaHost, model, whisperModel, profile, customPrompt) {
    responseHandler = null; // local mode routes transcripts to Ollama
...
module.exports = {
    ...
    sendLocalText,
    sendLocalImage,
    // transcription pipeline reuse (for other response backends)
    startTranscription,
    setResponseHandler,
